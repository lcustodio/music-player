...
describe('Play Queue List', () => {
  ...
  it('should display artist name', () => {
    const expected = shallow(<PlayQueue songs={songs()} />);
    expect(expected.at(0).at(0).html()).toContain('Artist 1');
  });

  it('should trigger callback when music is selected', () => {
    const mockCallback = jest.fn();
    const expected = shallow(<PlayQueue songs={songs()} onMusicSelected={mockCallback} />);
    expected.at(0).at(0).simulate('click');
